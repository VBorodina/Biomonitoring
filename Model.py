...
class Model:
    ...
    ,  p_given = None, datasets_dict_given = None, settings_dict_given = None
    ...
    ,**fit_kws ):           
        ...
        start = time.time()
        result = minimize(self.residuals, p, args=(datasets_dict,  settings_dict, kwargs_solve_ivp), method = method, nan_policy = nan_policy, **fit_kws)
        end = time.time()
    ...
    def prepare(self, experiments):
        ...
        if type(experiments) is dict:
            ...
            experiment_dict = {experiment.exp_nr : experiment for experiment in experiment_list}   
        ...
        for exp_nr, experiment in experiment_dict.items():
            self.datasets_dict[exp_nr] = experiment.dataset
            self.settings_dict[exp_nr] = self.create_settings(experiment)
...
#self.data_and_residuals = self.get_residuals(self.p, self.yields, datasets_dict, settings_dict, kwargs_solve_ivp)
        ...
        for exp_nr, dataset in datasets_dict.items():
            settings = settings_dict[exp_nr]
            ...
            residuals_dict_all[exp_nr] =residuals_dict_single
        
        return residuals_dict_all



    def statistics(self, residuals_dict_all, data_types = ["off", "on",  "CO2"]):
        ...
        for exp_nr, dataset in residuals_dict_all.items():
            RMSE = {}
            BIAS = {}
            STDDEV = {}
            stat = {}
            ...
            stat["BIAS"] = BIAS
            stat["STDDEV"] = STDDEV
            statistics_single_exp[exp_nr] = stat
