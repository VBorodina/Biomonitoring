...
class Yeast(Model):     #Dependent on base class
    ...
    def set_params(self, p = None):
        ...
        if p is None:          
            ...
            p.add("Yxs_ox", value=0.49, min=0.3, max=0.6, vary=True) #Yield biomass per glucose (oxidative growth) [g/g]  
    ...
    def calc_weighting_factors(self, experiment):
        ...
        for dskey in experiment.dataset.keys():
            weighting_factors[dskey] = len(experiment.dataset["off"])/len(experiment.dataset[dskey])
    ...
    , t_step = 1000, **kwargs_solve_ivp_given):  
        ...
        if experiment is not None:      #This if statement is to directly simulate from experiment, it is possible to give an experiment with modified t,y,p,c
            ...
            if t_grid is None:
                ...
                end = (experiment.metadata["end1"] - experiment.metadata["start"]) / pd.Timedelta(1,"h")        
                t_grid = np.linspace(0, end, t_step) #timeline for simulation, going from 0 to metadata["end1"]
    ...
    def residuals(self, p, datasets_dict, settings_dict, kwargs_solve_ivp):
            ...
            for dskey, dat in dataset.items():   #extract data for ("on", "off", "CO2") for each experiment
                ...
                for var in dat:     #loop over measured variables (columns)
                    if var in sim_exp.columns:
                        res_var = wf * (sim_exp[var] - dat[var]).values # weighted residuals for this measured variable
